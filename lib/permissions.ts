...
export function canViewLead(user: SessionData, lead: LeadOwnership): boolean {
  ...
  if (user.role === "COUNSELLOR") return lead.assignedCounsellorId === user.userId || lead.createdById === user.userId;
...
export function buildLeadWhereClause(user: SessionData) {
  ...
  if (user.role === "COUNSELLOR") return { OR: [{ assignedCounsellorId: user.userId }, { createdById: user.userId }], isArchived: false };
