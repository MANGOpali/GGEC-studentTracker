    ...
    <div className="min-h-screen bg-slate-100">
      <header className="sticky top-0 z-50 bg-white shadow-sm border-b border-gray-200">
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6 h-16 flex items-center gap-4">
          <Link href="/counsellor/dashboard" className="flex-shrink-0 mr-2">
          ...
          <nav className="hidden lg:flex items-center gap-1 flex-1">
            {navItems.map((item) => {
              const active = pendingHref
                ...
                : (pathname === item.href || (item.href !== "/counsellor/dashboard" && item.href !== "/leads/new" && pathname.startsWith(item.href)));
              return (
                <Link
                  key={item.href}
                  href={item.href}
          ...
          <div className="flex items-center gap-2 ml-auto lg:ml-0">
            ...
            <div className="hidden sm:flex items-center gap-2 pl-3 border-l border-gray-200">
              ...
              <div className="hidden md:block">
                ...
                <p className="text-xs text-gray-400">Counsellor</p>
              </div>
            </div>
