...
export default function FollowUpsClient({ showCounsellorFilter = false }: Props) {
  const [search, setSearch] = useState("");
  const [counsellorId, setCounsellorId] = useState("");

  const params = new URLSearchParams({ hasFollowup: "1", pageSize: "300" });
  if (counsellorId) params.set("counsellorId", counsellorId);

  const { data, isLoading } = useQuery({
    queryKey: ["followups", counsellorId],
    ...
    <div>
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Track and manage scheduled follow-ups</p>
        </div>
        ...
        {isLoading ? (
          ...
          <>
            <TabsContent value="overdue">
              <LeadList leads={overdue} urgent emptyMsg="No overdue follow-ups" showCounsellor={showCounsellorFilter} />
            </TabsContent>
            <TabsContent value="today">
              <LeadList leads={dueToday} emptyMsg="No follow-ups due today" showCounsellor={showCounsellorFilter} />
            </TabsContent>
            <TabsContent value="upcoming">
              <LeadList leads={upcoming} emptyMsg="No upcoming follow-ups" showCounsellor={showCounsellorFilter} />
            </TabsContent>
          </>
        )}
      </Tabs>
...
function LeadList({ leads, urgent = false, emptyMsg, showCounsellor }: {
  leads: Lead[];
  urgent?: boolean;
  emptyMsg: string;
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-gray-500 mt-0.5">
                ...
                {showCounsellor && lead.assignedCounsellor && (
                  <> · <span className="text-blue-600">{lead.assignedCounsellor.name}</span></>
                )}
              </p>
            </div>
          </div>
